 ...
 * Proxy (internal)
 */
class DateTimeProxy extends ValueProxy implements IXmlSchemaType
{
	public static $CLASSNAME = "lyquidity\XPath2\Proxy\DateTimeProxy";

	/**
	 * @var DateTimeValue $_value
	 */
	private $_value;
	...
	{
		return DateTimeProxyFactory::Code;
	}
	...
	{
		if ( $val->GetValueCode() != DateTimeProxyFactory::Code )
	...
	protected function Add( $value )
	{
		switch ( $value->GetValueCode() )
		{
			case YearMonthDurationProxyFactory::Code:
				...
				$clone = DateTimeValue::fromDate( $this->_value->S, $this->_value->Value, ! $this->_value->IsLocal );
				return new DateTimeProxy( DateTimeValue::AddYearMonthDuration( $clone, $value->getValue() ) );
				// return new DateTimeProxy( DateTimeValue::Add(  $this->_value, /* YearMonthDurationValue */ $value->getValue() ) );
			...
			case DayTimeDurationProxyFactory::Code:
				 ...
				 */
				// Clone the DateValue contained in the proxy or the original is changed
				$clone = DateTimeValue::fromDate( $this->_value->S, clone $this->_value->Value );
				return new DateTimeProxy( DateTimeValue::AddDayTimeDuration( $clone, $value->getValue() ) );
				// return new DateTimeProxy( DateTimeValue::Add(  $this->_value, /* DayTimeDurationValue*/ $value->getValue() ) );
	...
	protected function Sub($value)
	{
		switch ( $value->GetValueCode() )
		{
			case DateTimeProxyFactory::Code:
				return new DayTimeDurationProxy( DateTimeValue::Sub( $this->_value, /* DateTimeValue */ $value->getValue() ) );
			...
			case YearMonthDurationProxyFactory::Code:
				 ...
				 */
				// Clone the DateValue contained in the proxy or the original is changed
				$clone = DateTimeValue::fromDate( $this->_value->S, clone $this->_value->Value, ! $this->_value->IsLocal );
	...
	public static function tests()
	{
		...
		$int1 = new DateTimeProxy( DateTimeValue::Parse( "2009-10-11" ) );
		$int2 = new DateTimeProxy( DateTimeValue::Parse( "2009-10-12" ) );
		...
		$result = $execute( function() use( $int1, $int2 ) { return $int1->Sub( new DateTimeProxy( DateTimeValue::Parse("2008-01-01" ) ) ); } );
