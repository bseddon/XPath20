...
{	     
	...
	public function yyparseSafe ( $tok)
	{
		...
		catch (Exception)   
		{
			...
			throw new XPath2Exception ("XPST0003", "{$errorMsg} at line {$tok->LineNo} pos {$tok->ColNo}" );
		}
	}
	 ...
	 * @return object
	 */
	public function yyparseSafe ( $tok, $yyDebug)
	{
	    try
		{
...
NameTest 
   : QName
   {
      XmlQualifiedName qualifiedName = QNameParser.Parse((String)$1, 
	    $this->context->NamespaceManager, "", $this->context->NameTable);
...
Wildcard
   ...
   | NCName ':' '*'
   {
      ...
      string ns = $this->context->NamespaceManager.LookupNamespace(ncname);
      if (ns == null)
   ...
   | '*' ':' NCName 
   {
      $$ = XmlQualifiedNameTest::New( $this->context->NameTable->Add( $3), null );
   }
   ; 
...
FunctionCall
   : QName '(' ')'  
   {
      XmlQualifiedName identity = QNameParser.Parse((string)$1, $this->context->NamespaceManager, 
	     $this->context->NamespaceManager.DefaultNamespace, $this->context->NameTable);
...
ElementNameOrWildcard
   : ElementName
   {
      $$ = XmlQualifiedNameTest::New((XmlQualifiedName)QNameParser.Parse((string)$1, 
	     $this->context->NamespaceManager, $this->context->NamespaceManager.DefaultNamespace, $this->context->NameTable));
   }
   | '*'
   {
      $$ = XmlQualifiedNameTest::New( null, null );
...
SchemaElementTest
   ...
   {
      XmlQualifiedName qname = QNameParser.Parse((string)$3, $this->context->NamespaceManager, 
	     $this->context->NamespaceManager->DefaultNamespace, $this->context->NameTable);
      XmlSchemaElement schemaElement = (XmlSchemaElement)$this->context->SchemaSet->GlobalElements[qname];
...
SchemaAttributeTest
   : SCHEMA_ATTRIBUTE '(' AttributeName ')'
   {
      XmlQualifiedName qname = QNameParser.Parse((string)$3, $this->context->NamespaceManager, 
	     $this->context->NamespaceManager->DefaultNamespace, $this->context->NameTable);
      XmlSchemaAttribute schemaAttribute = (XmlSchemaAttribute)$this->context->SchemaSet->GlobalAttributes[qname];
